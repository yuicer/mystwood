...
| 页面 | 主要能力 |
| --- | --- |
| `pages/index/index` | 展示空间状态、进行中的约定和等待回应的约定；任务卡使用圆形人物占位头像，不把任务附件作为首页封面；长轮询刷新 |
| `pages/space/create` | 创建空间 |
...
- 首页任务卡使用本地圆形人物占位头像 `assets/icons/task-avatar.svg`，用于表达参与者；当前不新增微信头像授权。
- 任务附件保持圆角方形，在创建、详情和回复区域统一使用 `180rpx` 缩略图，不与圆形头像混用。
